...
export function computeMetricsForHeader({ header, eventRows, durationRows, latencyRows, abcRows }) {
    const result = { header, metrics: {} };
    const totalEvents = _.sumBy(eventRows, (r) => r.payload?.total_count || (r.payload?.events?.length || 0));
    ...
    if (durations.length)
        result.metrics.mean_duration_sec = _.mean(durations);
...
export function mergeIndirectSummaries({ indirectPayloads, headerMetrics }) {
