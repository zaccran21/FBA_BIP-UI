...
new Worker('synthesize', async (job) => {
    ...
    // Load indirects
    const indirectRes = await pool.query(`SELECT payload, source FROM indirect_submissions WHERE case_id=$1 ORDER BY created_at`, [caseId]);
    ...
    await pool.query(`INSERT INTO aggregated_summaries(case_id, summary, level) VALUES($1, $2, $3)
